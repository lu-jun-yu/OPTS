...
1. 只保留竞赛级来源：`olympiads`、`amc_aime`（去掉质量不稳定的 `aops_forum`）
2. 去掉超长 problem（>2000 字符）
3. 去掉超长 solution（>3000 字符，通常对应过难的题）
4. 去除与测试集重叠的题目（对比 AIME25、AMC23、MATH500、Minerva Math，防止数据污染）
5. 按 source 分层采样：amc_aime: 4000，olympiads: 6000
6. Answer parsability 过滤：通过 `math_verify.parse()` 验证答案可解析，确保 RLVR 训练时 reward 信号可靠（不可解析的答案会退化为字符串匹配）
...
| aops_forum     | ~30k  | 中-高 | 不推荐  | 论坛用户贡献，格式不统一，答案质量参差不齐 |
