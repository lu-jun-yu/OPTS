...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        from .core_algos_exp8_2 import compute_treegae_advantage_return
...
def select_next_states(
    ...
    # --- Global sort and select top max_search_batch_size ---
    candidates.sort(key=lambda x: x[0], reverse=True)
    selected = candidates[:max_search_batch_size]
...
class RayOPTSTTPOTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        batch_size = self.config.data.get("gen_batch_size", self.config.data.train_batch_size)
        max_search_ratio = float(self.config.algorithm.get("max_search_ratio", 1.0))
        if not 0.0 <= max_search_ratio <= 1.0:
            raise ValueError(f"algorithm.max_search_ratio must be in [0, 1], got {max_search_ratio}")
        max_search_batch_size = int(max_search_ratio * batch_size)
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_idx in range(len(self.train_dataloader)):
                ...
                metrics = {}
                timing_raw = {}
                metrics["opts_ttpo/max_search_ratio"] = max_search_ratio
                metrics["opts_ttpo/max_search_batch_size"] = max_search_batch_size
                        ...
                        if round_idx < n_rounds - 1:
                            with timed_block("select_next_states", step=self.global_steps, round_idx=round_idx):
                                selected_states = select_next_states(
                                    ...
                                    gamma=self.config.algorithm.gamma,
                                    max_prompt_length=self.config.data.max_prompt_length,
                                    max_search_batch_size=max_search_batch_size,
                                    tokenizer=self.tokenizer,
                                )
