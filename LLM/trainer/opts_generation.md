...
当前推理实现每个 step 都重新初始化 `global_batch`、`next_states`、`search_count` 和 `max_exploitations`，搜索门控只依赖 OTRC 自身的 exploitation/exploration 与 `max_exploitations` 均值筛选。

### 4.4 搜索门控

1. 每个 step 内维护 `max_exploitations`
2. `select_next_states` 只保留 exploitation 高于当前正值均值的候选树
3. 所有搜索状态都在 step 级别重置
