...
尽管每条 continuation 都由当前策略采样，但分支点由算法外部指定，并不属于标准 on-policy 链式采样过程。等权求和会放大分支点后的节点的梯度贡献：若 $u$ 后有 $b$ 条同分布 continuation，则 $\mathbb{E}[\sum_{j=1}^{b}G_j(u)\mid u]=b\,\mu(u)$，而在标准链式采样中原本只应贡献 $\mu(u)$。对于每个节点，该放大会随着祖先轨迹分支增多，沿祖先轨迹的分支数累乘。下文用 $W(x)$ 表示节点 $x$ 的分支放大因子，用 $1/W(x)$ 作为分支校正权重。
...
令 $\mathcal{B}$ 为当前 batch 的树节点集合，并使用 $1/W(x)$ 作为节点 $x$ 的分支校正权重。记 $\hat A_x=\hat A^{\mathrm{TreeGAE}}_x$，PPO ratio 为 $r(\theta)=\frac{\pi_\theta(a_x\mid s_x)}{\pi_{\theta_{\mathrm{old}}}(a_x\mid s_x)}$。
...
设当前贪心参考路径为 $\tau^\star=(s_0,a_0,r_0,\ldots,s_{n-1},a_{n-1},r_{n-1},s_n)$，其中位置 $t$ 对应树节点 $x_t$。若在位置 $k$ 后继续搜索，我们关心从 $s_k$ 重新产生后缀轨迹的期望回报相比当前贪心后缀能改进多少。定义后缀期望改进量
$$
\Delta_k^\pi:=V^\pi(s_k)-G_k,
\qquad
G_k=\sum_{t=k}^{n-1}\gamma^{t-k}r_t,
$$
其中 $G_k$ 是当前贪心后缀的实际 return，$V^\pi(s_k)$ 是从 $s_k$ 出发按策略产生后缀的期望 return。假设参考路径有限，且 $V^\pi(s_n)=0$，则
$$
\begin{aligned}
-\sum_{t=k}^{n-1}\gamma^{t-k}\delta^\pi_t
&= -\sum_{t=k}^{n-1}\gamma^{t-k}\bigg[r_t+\gamma V^\pi(s_{t+1})-V^\pi(s_t)\bigg]\\
...
**引理 1（后缀期望改进量的残差分解）** 对有限贪心参考路径，若 $V^\pi(s_n)=0$，则 $\Delta_k^\pi=-\sum_{t=k}^{n-1}\gamma^{t-k}\delta^\pi_t$。

该分解把“后缀期望改进”转化为沿参考路径估计 TD 残差。给定参考路径后，$s_t,a_t,r_t,s_{t+1}$ 均固定，因此 $\delta^\pi_t$ 是确定量。若直接用 $\hat\delta_t=r_t+\gamma\hat V(s_{t+1})-\hat V(s_t)$ plug-in 估计它，估计值完全由当前一步转移和 critic 决定，没有后续采样方差；但 $\hat V$ 对 $V^\pi$ 的偏差会直接进入每个残差。我们考虑利用 $s_{t+1}$ 之后的 on-policy continuation，引入采样随机性，来平衡偏差：条件于当前一步转移，若 $\hat V=V^\pi$，则 $\mathbb{E}[\hat A^{\mathrm{TreeGAE}}_t\mid s_t,a_t,r_t,s_{t+1}]=\delta^\pi_t$，所以它可作为 $\delta^\pi_t$ 的随机估计。据此，定义 OTRC 的开发项为
$$
...
1. 初始化全局树 batch 与待重分支节点集合，并进行 $R$ 轮搜索 rollout：
   - 第一轮从初始状态采样 $\mathrm{bs}$ 条链式轨迹；后续轮次根据上一轮 OTRC 选出的重分支节点生成新 continuation。若待重分支节点数少于 $\mathrm{bs}$，则用新状态补齐本轮 batch。
   - 生成新 continuation 后，将本轮样本并入全局树。
   - 在当前全局树上按照第 3.1.2 节的公式从后往前更新 TreeGAE。
   - 若当前不是最后一轮搜索，则先过滤已达到最大搜索次数的树；对其余树基于最新 TreeGAE 构造贪心参考路径并计算 OTRC，得到候选重分支节点 $x_{k^\star}$。随后按候选节点的期望改进执行平均基线过滤，仅保留高于当前平均期望改进的候选，作为下一轮待重分支节点。
2. 搜索结束后，根据第 3.1.1 节计算最终树节点的分支放大因子 $W(x)$，并以 $1/W(x)$ 为分支校正权重，按第 3.1.3 节构造 TTPO actor/critic 损失。
3. 使用 $\mathcal{L}_V^{\mathrm{TTPO}}$ 和 $\mathcal{L}_\pi^{\mathrm{TTPO}}$ 分别更新 critic 和 actor。
