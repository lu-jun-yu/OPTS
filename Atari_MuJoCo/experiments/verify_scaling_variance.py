...
def collect_opts_steps(env, agent, num_steps, device, max_search, c,
    ...
    return_threshold 使用当前已完成 episode 的 mean return。
    """
    env_idx = 0
    num_envs = 1
    ...
    current_parent = -1
    episodic_returns = []
    episodic_return_info = []  # (ep_return, tid, step)
    mean_return = None
    ...
    for step in range(num_steps):
        ...
        if done and "episode" in info:
            ...
            episodic_return_info.append((ep_return, tree_indices[step, env_idx].item(), step))
            tid = tree_indices[step, env_idx].item()
            if ep_return > tree_max_returns[0].get(tid, float('-inf')):
                tree_max_returns[0][tid] = ep_return

            # 实时计算 branch_weights 并用 IPW 加权算 mean_return
            cur_weights = compute_branch_weight(
                num_steps=step + 1,
                parent_indices=parent_indices[:step + 1],
                state_branches=state_branches[:step + 1],
                env_indices=[env_idx],
                root_branch_counts=root_branch_counts,
            )
            tid_groups = {}
            for er, t_id, t_step in episodic_return_info:
                if t_id not in tid_groups:
                    tid_groups[t_id] = []
                tid_groups[t_id].append((er, t_step))
            aggregated_returns = []
            for t_id, entries in tid_groups.items():
                weighted_sum = 0.0
                weight_sum = 0.0
                for er, t_step in entries:
                    w = cur_weights[t_step, 0].item()
                    weighted_sum += er / w
                    weight_sum += 1.0 / w
    ...
    print(f"  OPTS: {len(episodic_returns)} episodes, mean_return(IPW)={mean_return}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-steps", type=int, default=100000,
                        help="total steps to collect (must be > max batch_size)")
    ...
    ppo_obs, ppo_act, ppo_adv = load_or_collect_ppo(
        env_ppo, agent, args.num_steps, device, ppo_cache_path,
        args.gamma, args.gae_lambda)

    print("Computing g* from full PPO buffer...")
    g_star = compute_pg_gradient(agent, ppo_obs, ppo_act, ppo_adv, device)

    print(f"PPO bootstrap variance estimation ({args.num_bootstrap} resamples per B)...")
    ppo_variances = estimate_scaling_variance(
        agent, ppo_obs, ppo_act, ppo_adv, batch_sizes, g_star, device,
    ...
    print(f"OPTS bootstrap variance estimation ({args.num_bootstrap} resamples per B)...")
